...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(sigmoid(np.dot(u_o.T, v_c))) - \
        np.sum(np.log(sigmoid(-np.dot(u_k, v_c))))

    gradCenterVec = -(1-sigmoid(np.dot(u_o.T, v_c))) * u_o - \
        np.dot(-(1-sigmoid(-np.dot(u_k, v_c))).T, u_k)

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx, :] = (
        sigmoid(np.dot(u_o.T, v_c)) - 1) * v_c
    ...
    gradOutsideVecs[unique, :] = - \
        ((sigmoidUniqueSamplesCenter - 1) * counts)[:, None] * v_c[None, :]
    # END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        gradloss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(
